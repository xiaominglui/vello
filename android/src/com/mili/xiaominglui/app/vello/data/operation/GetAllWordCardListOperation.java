...
public class GetAllWordCardListOperation implements Operation {
    private static final String TAG = GetAllWordCardListOperation.class.getSimpleName();
    ...
    public Bundle execute(Context context, Request request) throws ConnectionException,
        ...
        if (wordCardListSize > 0) {
            ...
            for (WordCard wordCard : wordCardList) {
                operationList.add(ContentProviderOperation.newInsert(DbWordCard.CONTENT_URI)
                        .withValues(wordCard.toContentVaalues()).build());
            }

            try {
                context.getContentResolver().applyBatch(VelloProvider.AUTHORITY, operationList);
